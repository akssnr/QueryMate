...
def _default_schema(index_name: str, text_key: str, sector : str, sub_sector : str, sales_person : str, sheet_name : str, table_name : str) -> Dict:
    return {
        "class": index_name,
        "properties": [
            {
...
def _json_serializable(value):
    # Implement this function to ensure values are JSON serializable
...
class Weaviate(VectorStore):
    ...
    def add_texts(
        ...
        embeddings: Optional[List[List[float]]] = None
        if self._embedding:
            if not isinstance((texts,sector,sub_sector,sales_person), list):
                texts = list(texts)
                sector = list(sector)
                sub_sector = list(sub_sector)
                sales_person = list(sales_person)
            embeddings = self._embedding.embed_documents((texts,sector,sub_sector,sales_person))
        from rich import print
        with self._client.batch as batch:
            for i, text in enumerate((texts,sector,sub_sector,sales_person)):
                # Assuming `texts` is a list of dictionaries with the required properties
                print("------------------Start---------------------")
                print(i,text)
                print("-----------------running--------------------")
                data_properties = {
                    # self._sector : sector,
                    # self._sub_sector : text["Sub_Sector"],
                    # self._sales_person : text["Sales_Person"],
                    self._text_key : text,
                    self._sector : text,
                    self._sub_sector : text,
                    self._sales_person : text,
                    # "Sheet_Name": text["Sheet_Name"],
                    # "Table_Name": text["Table_Name"],
                    # "Values_Table": text["Values_Table"],
                    # "Sector": text["Sector"],
                    # "Sub_Sector": text["Sub_Sector"],
                    # "cat": text["Sector"],
                    # "subcat": text["Sub_Sector"],
                    # "person": text["Sales_Person"],
                    # "sheetname": text["Sheet_Name"],
                    # "tablename": text["Table_Name"],
                    # "valuestable": text["Values_Table"],
                    # "excelmetadata": {
                    #     "source": text["Table_Name"][10:] if text["Table_Name"] else None,
                    #     "page": text["Table_Name"][:8] if text["Table_Name"] else None
                    # }
                    # "PDF_Content": text.get("PDF_Content"),
                    # "PDF_MetaData": text.get("PDF_MetaData")
                }
                print("-----------end---------------")
                if metadatas is not None:
                    for key, val in metadatas[i].items():
        ...
        **kwargs: Any,
        ...
        with client.batch as batch:
            for i, text in enumerate(texts):
                data_properties = { 
                    text_key : text,
                    # "Sector": text.get("Sector"),
                    # "Sub_Sector": text.get("Sub_Sector"),
                    # "Sales_Person": text.get("Sales_Person"),
                    # "Sheet_Name": text.get("Sheet_Name"),
                    # "Table_Name": text.get("Table_Name"),
                    # "Values_Table": text.get("Values_Table").to_dict() if isinstance(text.get("Values_Table"), pd.DataFrame) else text.get("Values_Table"),
                    # "Excel_Metadata": {
                    #     "source": text.get("Table_Name")[10:] if text.get("Table_Name") else None,
                    #     "page": text.get("Table_Name")[:8] if text.get("Table_Name") else None
                    # },
                    # "PDF_Content": text.get("PDF_Content"),
                    # "PDF_MetaData": text.get("PDF_MetaData")
                }
